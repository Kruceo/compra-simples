...
// 1 - precisa haver um fornecedor 

const fornecedor = await Fornecedor.create({
    nome: getRandomName()
})

// 2 - precisa haver um bote ao menos 
...
const entrada = await Entrada.create({
    ...
    obs: "Teste",
    status: 0,
    fornecedor_id: bote.dataValues.fornecedor_id,
...
const item2 = await Entrada_item.create({
    ...
    produto_id: produto2.dataValues.id,
    entrada_id: entrada.dataValues.id,
})
...
const item3 = await Entrada_item.create({
    ...
    entrada_id: entrada.dataValues.id,
    abater_valor: true                          // desconto coloca o produto de maneira que diminuirá o valor final enves de aumentar, 
})
