...
export default function FormSelection(props: FormSelectionAttributes) {
    ...
    return <>
        ...
        <select ref={ref} {...restProps}
            ...
            className={`bg-transparent px-3 py-2 border-borders border outline-none ${errored ? "border-red-600" : ""} ${props.className} focus:-outline-offset-2 outline-2 focus:outline-white ${loadingData ? "!h-0 !w-0 !appearance-none !p-0 !border-none absolute" : ""}`}>
            {
                props.children
            }
            {
