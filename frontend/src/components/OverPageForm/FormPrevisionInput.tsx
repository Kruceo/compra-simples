...
function Sugestions(props: { data: BackendTableComp[], itemHandler: (item: BackendTableComp) => React.ReactNode, inputRef: React.RefObject<HTMLInputElement>, isVisible: boolean, setter: (item: any) => void }) {
    ...
    useEffect(()=>{
        props.inputRef.current?.addEventListener("focus",(e)=>{
            setClicked(false)
        })
    })

    return <div className={`bg-subpanel border-borders border z-40 shadow-lg absolute top-full left-0 w-full flex flex-col justify-start m-o ${(props.isVisible || hovering) && !clicked ? "visible" : "invisible"}`}
