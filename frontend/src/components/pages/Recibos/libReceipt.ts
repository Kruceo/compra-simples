...
import { writeBox } from "../Relatorios/libraryReports";
...
export function WriteReceipt2PDF(pdf: jsPDF, y: number, value: number, receiverName: string, date: Date, referenceText?: string, n?: number) {
    function extensor(b: number) {
        ...
        let options: any = { mode: 'currency', currency: { type: 'BRL' }, number: { decimal: 'formal' } }
        return extenso(num, options)
    }
    let startFontSize = pdf.getFontSize()

    let outerBox = writeBox(pdf, 5, y, pdf.internal.pageSize.width - 10, 80)

    let valueBox = writeBox(pdf, outerBox.x2 - 50, outerBox.y + 5, 50, 7)

    pdf.text("R$", valueBox.x - 10, valueBox.y2 - 1.5)

    pdf.text(beautyNumber(value), valueBox.x + 1, valueBox.y2 - 1.5)

    let nBox = writeBox(pdf, outerBox.x + 45, valueBox.y, 25, 7)
    ...
    let line1Box = writeBox(pdf, outerBox.x, outerBox.y + 35, outerBox.w, 5)
    let line2Box = writeBox(pdf, outerBox.x, line1Box.y2, outerBox.w, 5)

    let lines = []
    let defaultLineY = line1Box.y - 1
    if (referenceText) {
        let text = "Referente a: " + referenceText
        for (let i = 0; i < 3; i++) {
